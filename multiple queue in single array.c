...
typedef struct {
    ...
    int front[NUM_QUEUES];   // Array to store front indices of each queue
    int rear[NUM_QUEUES];    // Array to store rear indices of each queue
    int size;                // Maximum size of each queue
} MultiQueue;

void initializeMultiQueue(MultiQueue *mq, int numQueues, int size) {
    mq->size = size;
    for (int i = 0; i < numQueues; i++) {
        mq->front[i] = i * size;    // Front pointer for each queue
        mq->rear[i] = i * size - 1; // Rear pointer for each queue
    }
}
